...
/**
 * OpenAI Compatible Response API Connector
...
export class OpenAICompatibleConnector implements LLMConnector {
  ...
  constructor(options: {
    ...
    this.defaultModel = options.model || 'qwen/qwen3-4b-thinking-2507';
  }
  ...
  async streamNextStep(
    ...
    try {
      ...
      throw new Error(`OpenAICompatible API error: ${error.message}`);
    }
  }
   ...
   * 
   * Note: This method returns the FIRST function call found in the output array.
   * If multiple function calls are present, they will be executed sequentially
   * through subsequent agent loop iterations.
   */
  private parseResponse(response: any): LLMResponse {
    // Check for tool calls in output
    const output = response.output || [];

    // Extract reasoning first (will be included in the response)
    let reasoning: string | undefined;
    const reasoningItem = output.find((item: any) => item.type === 'reasoning');
    if (reasoningItem) {
      // Reasoning can be in content array or directly as content
      if (Array.isArray(reasoningItem.content)) {
        reasoning = reasoningItem.content
          .filter((c: any) => c.type === 'reasoning_text')
          .map((c: any) => c.text)
          .join('');
      } else if (typeof reasoningItem.content === 'string') {
        reasoning = reasoningItem.content;
      }
    }

    // FIRST PASS: Look for function calls (highest priority)
    for (const item of output) {
      // Handle function_call type (direct function call format)
    ...
    // SECOND PASS: Look for messages (only if no function calls found)
    for (const item of output) {
